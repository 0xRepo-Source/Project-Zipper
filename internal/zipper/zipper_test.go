...
func TestGzipWithProgress(t *testing.T) {
	tmp := t.TempDir()
	root := filepath.Join(tmp, "src")
	if err := os.Mkdir(root, 0o755); err != nil {
		t.Fatalf("failed to create src dir: %v", err)
	...
	stats, err := GzipWithProgress(root, archive, func(done, total int64) {
		calls = append(calls, struct {
			done  int64
			total int64
		}{done: done, total: total})
	...
	if err != nil {
		t.Fatalf("GzipWithProgress returned error: %v", err)
	}
...
func TestExtractGzipWithProgress(t *testing.T) {
	tmp := t.TempDir()
	...
	stats, err := ExtractGzipWithProgress(archive, extractDir, func(done, total int64) {
		calls = append(calls, struct {
			done  int64
			total int64
		}{done: done, total: total})
	...
	if err != nil {
		t.Fatalf("ExtractGzipWithProgress returned error: %v", err)
	}
